...
$sql_check = "SELECT * FROM property_owner WHERE email=?";
...
if (mysqli_num_rows($result_check) > 0) {
    // Update password
    $sql_update = "UPDATE property_owner SET password=? WHERE email=?";
    $stmt_update = mysqli_prepare($conn, $sql_update);
    mysqli_stmt_bind_param($stmt_update, "ss", $new_password, $email);

    if (mysqli_stmt_execute($stmt_update)) {
        echo "<script>alert('Password updated successfully!'); window.location.href='propertyOwnerLogin.html';</script>";
    } else {
